...
const PromoScroller = ({ categoryId = 'home' }) => {
  ...
  useEffect(() => {
    const fetchPromos = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/promos/${categoryId}`);
        setPromos(res.data);
      } catch (err) {
  ...
  if (!promos.length) return <div>No promos found.</div>;
          ...
          >
            <Box>
              ...
              <Typography variant="body2" fontSize={16} color="#484848" mt={1}>
                {promo.description?.[currentLang] || promo.description?.en || ""}
              </Typography>
